...
Concretely, the repository includes representative JSON outputs (e.g., summary statistics, candidate data, and an exported vertex instance) under \texttt{mathematica/results/}, together with the Python and Lean scripts that consume them.
...
\caption{Coefficients $a_i$ of the computationally identified candidate that is later certified (in Lean) to be a vertex of the finite-dimensional feasibility polytope. Several coefficients saturate imposed box constraints, reflecting the role of auxiliary bounds used to keep the linear program numerically well-conditioned.}
