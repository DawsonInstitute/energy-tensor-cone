...
Analytic QEIs often yield derivative-based bounds (for example, Fewster's general framework \cite{fewster2000}), which have different scaling behavior; Figure~\ref{fig:bound-comparison} compares the proxy bound used in our search to a representative analytic scaling for Gaussian sampling.
...
Concretely, the repository includes representative JSON outputs under \texttt{mathematica/results/} (e.g., \texttt{summary.json}, \texttt{near\_misses.json}, \texttt{top\_near\_misses.json}, \texttt{violations.json}, and \texttt{vertex.json}), together with the Python and Lean scripts that consume them.
...
\caption{Coefficients $a_i$ of the computationally identified candidate that is later certified (in Lean) to be a vertex of the finite-dimensional feasibility polytope. Several coefficients saturate imposed box constraints (\emph{e.g.}, $a_i\approx \pm 100$), reflecting the role of auxiliary bounds used to keep the linear program bounded and numerically well-conditioned. The resulting near-binary activation pattern is consistent with a polyhedral vertex determined by a small active set of constraints.}
