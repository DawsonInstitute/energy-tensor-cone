...
Concretely, the repository includes representative JSON outputs under \texttt{mathematica/results/} (including the certified \texttt{vertex.json}), together with the Python and Lean scripts that consume them.

\subsection{Optimization Objective}

We solve a sequence of linear programs (LPs), each of the form
\begin{equation}
\min_{a \in \mathbb{R}^N} \; c \cdot a \quad \text{subject to} \quad
L_\gamma \cdot a \geq -B_\gamma \;(\forall \gamma \in \Gamma_{\text{sample}}),
\quad |a_i| \leq 100,
\end{equation}
...
\item Ten key theorems: \texttt{AQEIFamilyInterface.admissibleSet\_isClosed}, \texttt{AQEIFamilyInterface.admissibleSet\_convex}, \texttt{AffineToCone.homCone\_isClosed}, \texttt{AffineToCone.homCone\_convex}, \texttt{PolyhedralVertex.full\_rank\_active\_implies\_vertex}, \texttt{VertexVerificationRat.det\_nonzero}, \texttt{VertexVerificationRat.full\_rank\_kernel\_trivial}, \texttt{FinalTheorems.candidate\_active\_binding}, \texttt{FinalTheorems.Candidate\_Is\_Extreme\_Point}, and \texttt{ExtremeRays.extreme\_point\_def\_iff}.
